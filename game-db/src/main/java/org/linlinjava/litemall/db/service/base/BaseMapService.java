...
public class BaseMapService{
    ...
    @Cacheable(cacheNames="Map", key="#id")
	public Map findById(int id) {
        return mapper.selectByPrimaryKeyWithLogicalDelete(id,false);
    }
    @Cacheable(cacheNames="Map", key="#id",condition="#result.deleted == 0")
    ...
    @CachePut(cacheNames="Map", key="#map.id")
	public int updateById(Map map) {
        map.setUpdateTime(LocalDateTime.now());
        return mapper.updateByPrimaryKeySelective(map);
    }
    
   	@CacheEvict(cacheNames="Map", key="#id")
